...
def _record_id(record: Dict[str, Any]) -> str:
    for key in ("recordid", "id", "record_id"):
...
def fetch_opendatasoft_json(
    ...
    *,
    ...
    if not isinstance(data, dict):
        raise ValueError("Opendatasoft response must be a JSON object")
        ...
        rows.append(
            {
                ...
                "feed_url": source.url,
                "record_id": record_id,
                "field_summary": json.dumps(fields, separators=(",", ":"), ensure_ascii=False),
...
def ingest_opendatasoft_explore(
    *, config_path: Path, source_name: str, data_dir: Path, db_path: Path
) -> None:
    """Fetch Opendatasoft Explore API records and store them locally."""
    from ..storage.duckdb_store import init_db, upsert_opendatasoft_records, log_run
    ...
    try:
        ...
        n_new = upsert_opendatasoft_records(db_path, normalized)
