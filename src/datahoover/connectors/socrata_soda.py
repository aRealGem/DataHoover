...
def fetch_socrata_json(
    ...
    *,
    ...
    if not isinstance(data, list):
        raise ValueError("Socrata response must be a JSON array")
        ...
        rows.append(
            {
                ...
                "raw_json": json.dumps(record, separators=(",", ":"), ensure_ascii=False),
...
def ingest_socrata_soda(*, config_path: Path, source_name: str, data_dir: Path, db_path: Path) -> None:
    """Fetch Socrata SODA JSON and store it locally."""
    from ..storage.duckdb_store import init_db, upsert_socrata_records, log_run
    ...
    try:
        fr = fetch_with_retry(
            lambda: fetch_socrata_json(source.url, etag=state.get("etag"), last_modified=state.get("last_modified"))
        )
        init_db(db_path)
        ...
        n_new = upsert_socrata_records(db_path, normalized)
