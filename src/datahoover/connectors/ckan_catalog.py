...
def _normalize_packages(
    source: Source, packages: List[Dict[str, Any]], ingested_at: datetime
) -> List[Dict[str, Any]]:
    rows: List[Dict[str, Any]] = []
    for pkg in packages:
        org = pkg.get("organization") or {}
        package_id = pkg.get("id") or str(uuid.uuid4())
        ...
        rows.append(
            {
                ...
                "name": pkg.get("name"),
                "title": pkg.get("title"),
                "organization": org.get("name") or org.get("title"),
                ...
                "raw_json": json.dumps(pkg, separators=(",", ":"), ensure_ascii=False),
...
def ingest_ckan_catalog(*, config_path: Path, source_name: str, data_dir: Path, db_path: Path) -> None:
    """Fetch CKAN package_search results and store them locally."""
    from ..storage.duckdb_store import init_db, upsert_ckan_packages, log_run
    ...
    try:
        fr = fetch_with_retry(
            lambda: fetch_ckan_json(source.url, etag=state.get("etag"), last_modified=state.get("last_modified"))
        )
        init_db(db_path)
        ...
        normalized = _normalize_packages(source, packages, ingested_at)
        n_new = upsert_ckan_packages(db_path, normalized)
