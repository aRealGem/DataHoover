...
def _parse_dt(value: str | None) -> datetime | None:
    if not value:
        return None
    # "2024-08-09T00:00:00.000Z" -> "+00:00"
...
def _normalize_records(
    ...
    rows = []
    for rec in records:
        declaration_id = rec.get("femaDeclarationString") or str(uuid.uuid4())
        rows.append(
            {
                "source": source.name,
                ...
                "declaration_date": _parse_dt(rec.get("declarationDate")),
                "incident_type": rec.get("incidentType"),
                "declaration_title": rec.get("declarationTitle"),
                "incident_begin_date": _parse_dt(rec.get("incidentBeginDate")),
                "incident_end_date": _parse_dt(rec.get("incidentEndDate")),
...
def ingest_openfema_disaster_declarations(
    *, config_path: Path, source_name: str, data_dir: Path, db_path: Path
) -> None:
    """Fetch OpenFEMA DisasterDeclarationsSummaries and store it locally."""
    from ..storage.duckdb_store import init_db, upsert_openfema_disaster_declarations, log_run
    ...
    try:
        fr = fetch_with_retry(
            lambda: fetch_openfema_json(source.url, etag=state.get("etag"), last_modified=state.get("last_modified"))
        )
        init_db(db_path)
        ...
        normalized = _normalize_records(source, records, ingested_at)
        n_new = upsert_openfema_disaster_declarations(db_path, normalized)
